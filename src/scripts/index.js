...
const searchWindow = document.querySelector(".search_input");
...
function markImportant(e) {
  const btn = e.target.closest(".btnImportant");
...
document
  ...
  .addEventListener("click", e => markImportant(e));
...
function searchTasksFromInput(searchWindow, parentName) {
  for (let x = 0; x <= parentName.children.length - 1; x++) {
    if (
      parentName.children[x].innerHTML
        .toLowerCase()
        .includes(searchWindow.value.toLowerCase())
    ) {
      ...
      for (let y = 0; y <= todo.children.length - 1; y++) {
        if (
          todo.children[y]
            ...
            .classList.contains("btnImportant")
        ) {
